...
for prog in topLevelTuple:
    if len(re.findall('[a-z]', prog[1])) > 1:
        namesWeightsRelations[re.findall('[a-z]+',prog[0])[0]] = (int(re.findall('\d+', prog[1])[0]),
                                                                  re.findall('[a-z]+', prog[1]))
    else:
        namesWeightsRelations[re.findall('[a-z]+', prog[0])[0]] = (int(re.findall('\d+', prog[1])[0]),
                                                                   '')
...
for name, values in zip(namesWeightsRelations.keys(), namesWeightsRelations.values()):
